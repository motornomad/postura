...
def test_parse_db_file():
    ...
    nodes, edges, accesses, _imports, _flows = parse_file(str(FIXTURE_DIR / "db.py"), str(FIXTURE_DIR))
...
def test_parse_app_file():
    ...
    nodes, edges, accesses, _imports, _flows = parse_file(str(FIXTURE_DIR / "app.py"), str(FIXTURE_DIR))
    names = [n.name for n in nodes]
...
def test_function_line_numbers():
    ...
    nodes, _, _a, _i, _f = parse_file(str(FIXTURE_DIR / "db.py"), str(FIXTURE_DIR))
...
def test_call_edges_extracted():
    ...
    _, edges, _a, _i, _f = parse_file(str(FIXTURE_DIR / "app.py"), str(FIXTURE_DIR))
...
def test_decorator_extraction():
    ...
    nodes, _, _a, _i, _f = parse_file(str(FIXTURE_DIR / "app.py"), str(FIXTURE_DIR))
...
def test_datastore_detection_db_file():
    ...
    _, _, accesses, _i, _f = parse_file(str(FIXTURE_DIR / "db.py"), str(FIXTURE_DIR))
...
def test_import_packages_extracted():
    ...
    _, _, _, imports, _f = parse_file(str(FIXTURE_DIR / "app.py"), str(FIXTURE_DIR))
    assert "flask" in imports, f"Expected 'flask' in imports, got {imports}"

    _, _, _, db_imports, _f2 = parse_file(str(FIXTURE_DIR / "db.py"), str(FIXTURE_DIR))
